...
var tableBuilder = {
    ...
    {
        if(x === 0 && y === 0) {
            let diff = this.maxX - this.minX + 1;
            return ''
                + '<th class="restart">'
                    + '<a href="#" '
            ...
            let classX = 'x-' + x;
            let classY = 'y-' + y;
            let tabIndex = this.getTabIndex(x, y);
    ...
    test: function(el, x, y, testGrid)
    {
        ...
        if(! answer || isNaN(answer)) {
            ...
            let newGoodAnswer = el.classList.contains('good') ? false : true;
            if(answer === test) {
                el.classList.add('good');
                el.classList.remove('bad');
                this.myCookie.setCookie(el.id, answer);
                if(newGoodAnswer) {
                    let newTabIndex = this.getNextTabIndex(x, y);
                    if(newTabIndex) {
                        newTabIndex.focus();
                    }
                }
    ...
    {
        let selector = 'x-'+x;
        let cells = document.getElementsByClassName(selector);
        let i = 0;
    ...
    getNextTabIndex:function(x ,y)
    {
        ...
        let getNextTabIndexValue = this.getTabIndex(x, y);
        let selector = 'input[tabindex=\''+getNextTabIndexValue+'\']';
